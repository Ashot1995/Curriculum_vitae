(function(window, document, $, undefined) {
    ...
    var usName = function(){
        ...
        if( $("#username").val() == "" ){
            ...
            $("#errUser").html("*Please write your valid username ! ");
            $('#username').css('border', '1px solid red')
        }
    };
...
//             $('#errText').html("*Please write this text !");
//         }
//         else{
//             $("#numb").css("border","1px solid green");
//             $('#errText').html("");
//         }
//     };
    ...
    $("#email").click(function(){
        ...
        $.ajax({
            type:'POST',
            url: "forgotPassword/Captcha/php/user.php",
            data: username ,
            dataType: 'json',
    ...
    $("#numb").click(function(){
        ...
        $.ajax({
            type:'POST',
            url: "forgotPassword/Captcha/php/dbuser.php",
            data: {
                'username': username,
                'email': email
            },
            ...
            success: function (response) {
                ...
                if(response['result'] == 'false_email' ){
                    $("#username").css("border","1px solid green");
                    $('#errUser').html("");
                ...
                if(response['result'] == 'false_username' ){
                    $("#username").css("border","1px solid red");
                    $('#errUser').html("*Please write your valid username !");
                ...
                if(response['result'] == 'false_email_username' ){
                    $("#username").css("border","1px solid red");
                    $('#errUser').html("*Please write your valid username !");
    ...
    var CAPTCHA = function(config) {
        ...
        var canvasWrapper = $('<div class="images">').prependTo(this._container);

        this._canvas = $('<canvas>').appendTo(canvasWrapper)
        // .attr("width", this._settings.width).attr("height", this._settings.height);
                ...
                success: function (response) {
                    ...
                    if(response['result'] == 'true' ){
                        ...
                        if(  userText === text ){
                            ...
                            alert("Your new password sent in mail")
                            window.location.href = ("index.php");


                        }else {
                            $("#numb").css("border","1px solid red");
                            $('#errText').html("*Captcha incorrect, please try again !");
                        }
